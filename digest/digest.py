...
# Skip MiniLM on tiny boxes only. 768MiB available used to fail-open the 3.5GiB
# Aliyun host under load even though total RAM is enough for all-MiniLM-L6-v2.
PHASE2_MINILM_SKIP_TOTAL = 2 * 1024 * 1024 * 1024
PHASE2_MINILM_SKIP_AVAIL = 256 * 1024 * 1024
...
# Original-case tokens from the last resolve, so the phase-1 reminder can
# name Willie rather than the casefolded allow-list.
_decision_alias_display: tuple[str, ...] = ()
_ALIAS_CACHE_FILENAME = ".decision-user-aliases.json"
...
def _decision_user_subjects(
    oneshot: Callable[..., dict[str, Any]] | None = None,
) -> frozenset[str]:
    """Resolve subjects before the phase-1 prompt, not after the model emits.

    A name/heading regex handed aliases to the validator too late, so the
    model only ever wrote ``user``. Reuse a saved list while USER.md's hash
    matches; a missing hash or missing ``aliases`` key scans again. A
    256-token cap fills the whole completion before ``submit_user_aliases``
    is emitted, so the scan returns user-only and writes nothing.
    """
    global _decision_alias_display
    base = frozenset(_DECISION_USER_SUBJECTS_BASE)

    def _apply(display: tuple[str, ...]) -> frozenset[str]:
        global _decision_alias_display
        _decision_alias_display = display
        folded = {token.casefold() for token in display if token.casefold() != "user"}
        return base | folded
    ...
    try:
        ...
        return _apply(())
    current_hash = _content_hash(text_u)
    cache_path = user_path.parent / _ALIAS_CACHE_FILENAME
    caller = oneshot or run_worker_llm_oneshot
    ...
    with _digest_lock:
        ...
        if cache_path.is_file():
            ...
            if (
                ...
                and str(data.get("user_md_sha256") or "") == current_hash
                and "aliases" in data
            ):
                cache_hit = True
                raw_saved = data.get("aliases")
                if raw_saved is None:
                    cached = ()
                elif isinstance(raw_saved, list):
                    cached = tuple(
                        str(token).strip()
                        for token in raw_saved
                        if str(token).strip() and str(token).strip().casefold() != "user"
                    )
                else:
                    cache_hit = False
        if cache_hit and cached is not None:
            return _apply(cached)

        try:
            capture = caller(
                (
                    "Find personal name aliases the user goes by, for the "
                    "grammatical subject of a preference line. Return short "
                    "name tokens only: given name, English name, or nickname. "
                    "Do not return roles, schools, section titles, or the word "
                    "user. If no personal alias is stated, set aliases to null.\n\n"
                    "USER.md:\n"
                    f"{text_u}"
                ),
                plugin="memory-digest",
                purpose="user-alias-scan",
                force_tool_name="submit_user_aliases",
                tool_schema={
                    "name": "submit_user_aliases",
                    "description": (
                        "Names the user goes by, or null when USER.md states none."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            return _apply(())
        if not isinstance(capture, dict) or capture.get("failed"):
            return _apply(())
        tool_args = capture.get("tool_args")
        if not isinstance(tool_args, dict) or "aliases" not in tool_args:
            return _apply(())
        raw_aliases = tool_args.get("aliases")
        if raw_aliases is None or raw_aliases == []:
            ...
            for item in raw_aliases:
                ...
                if not token or token.casefold() == "user" or token in cleaned:
                    continue
                cleaned.append(token)
                if len(cleaned) >= 8:
                    break
            display = tuple(cleaned)
        else:
            return _apply(())
        payload = {
            "user_md_sha256": current_hash,
            "aliases": list(display) if display else None,
        }
        try:
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            fd, tmp_name = tempfile.mkstemp(
                dir=str(cache_path.parent), suffix=".tmp", prefix=".alias_"
            )
            try:
...
- decision: user-only rulings and preferences for agent behavior. Mirror the transcript's intensity; do not normalise the ruling to must. Examples, not a closed list: intense must / must not (write forbids / banned as must not); less intense prefer / prefers; neutral locked / decided / chose / sets; etc. Use another verb when it fits the session history's intensity better. Identify who must act. The user's own preference or decision stays about the user: "I prefer tea" → user prefers tea; "I decided X" → user decided X. An expectation of the agent names the agent: "don't parrot me" → user wants the agent not to parrot the user's words; "send the report" → user wants the agent to send the report. Do not write user must for an order to the agent. Including decision_constraint: user feedback on that procedure, corrections, and standing prefs. Emit subject plus a predicate ruling so Preference:/Decision: {{subject}} {{ruling}} is one clause. First subject after Preference:/Decision: must be user/User. Use only user unless the decision slot reminder names aliases. Third-party traits/living/likes → fact (`Narration:` / kind=Narration), even when the user reported them. Do not duplicate the full procedure or event summary. Corrections use supersedes: + confidence: explicit. `decision_constraint` is accepted only as a legacy input alias and is normalized to canonical `decision`.
...
def _weekly_clock_leftover_flag(local: datetime, need_leftover: bool) -> bool:
    """Pass leftover_ran after 23:55 Sunday so the weekly clock can see leftover finished.

    Close itself waits for Monday 00:00 evening patch.
    """
...
def _phase2_prompt_and_tool(
    ...
    *,
    ...
    Skip MiniLM only on ≤2GiB boxes or <256MiB available so a 3.5GiB VPS can
    still filter; embed/math failure fail-opens to the full board. Attempt ≥2
    still shrinks to foul-touched cards only.
    """
            ...
            if missing_i:
                if _PHASE2_MINILM is None:
                    ...
                    skip_ram = (
                        ...
                        or (0 < mem_total <= PHASE2_MINILM_SKIP_TOTAL)
                        or (0 < mem_avail < PHASE2_MINILM_SKIP_AVAIL)
                    )
                    if skip_ram:
                        msg = (
                            "phase2 Pearson/MI gate skipped MiniLM (ram) "
                            f"total_mib={mem_total // (1024 * 1024)} "
                            f"avail_mib={mem_avail // (1024 * 1024)}"
                        )
                        _log(msg)
...
def _worker_type_body_reminder(worker_type: str) -> str:
    """Keep the rendered body shape in front of the worker that fills that type.

    Decision wording names the agent when the user is ordering the assistant.
    Without that sample the card reads as a rule the user must follow.
    """
    assigned = LEGACY_TYPE_ALIASES.get(worker_type, worker_type)
    if assigned == "event":
        return (
    ...
    if assigned == "decision":
        _decision_user_subjects()
        names = ", ".join(_decision_alias_display)
        subject = f"`user` or {names}" if names else "`user`"
        return (
            f"Slot reminder: kind=Preference|Decision, subject={subject}, "
            "ruling=predicate that mirrors the transcript intensity; do not "
            "normalise to must. Examples, not a closed list: intense must / "
            "must not (write forbids / banned as must not); less intense "
            "prefer / prefers; neutral locked / decided / chose / sets; etc. "
            "Use another verb when it fits the session history's intensity "
            "better. Identify who must act. Own preference stays about the "
            "user (user prefers tea). An expectation of the agent is "
            "user wants the agent to send the report, or user wants the "
            "agent not to parrot the user's words. Do not write user must "
            "for an order to the agent. Do not repeat subject. First subject "
            "after Preference:/Decision: must be user/User. Look in the "
            "transcript for the user's rulings. Third-party traits → fact "
...
def on_pre_llm_call(
    ...
    **_: Any,
    ...
    inbound user 13 is already in sqlite at pre-LLM time. Bands return on every
    non-cron call because Hermes fences prefetch per turn; a once-a-day skip
    left later turns and compact without that index.
    """
    ...
    if not text:
        return None
    _log(f"recall injected session={session_id or '-'} window=bands-a-c")
    return {"context": text}
