...
_SYSTEM_PROMPT_BLOCK = (
    ...
    "preferences/procedures) inject once per civil day. Band B names are the "
    "last-7-day daily entity index. Band C entities: lines on older weeks are "
    "weekly.md canonical names, not card bodies. Matching preference "
    "is a constraint; matching procedure is a reusable case. If no monthly "
    "guidance admits, search daily decision/procedure cards only. For a "
    "week-shaped question, read Band C summary bullets first; map a matching "
    "bullet's weekday (Monday = that week's printed start) to one civil ISO "
    "date and call recall_memory with time_from and time_to set to that day, "
    "not the week span — optionally use a Band C canonical name as query. "
    "Prefer event cards in that day's dump; if a fact, procedure, or decision "
    "is needed, recall_memory a related: id or expand_memory on the event. "
    "If Channel 4 (embed) locates a month, week, or daily match, the same "
    "recall_memory call fills the card via mem-id or that civil day. "
    "If a month block matches, pass that block's printed ISO start and end. "
    ...
    "card corpus. MEMORY.md and USER.md are already in the system prompt — do "
    "not expect those hot files in the bands."
)
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        even when those contexts skip the clock. Re-patch prefetch join here
        because MemoryManager is often constructed before this plugin loads.
        """
        self._session_id = str(session_id or "")
        self._hermes_home = str(kwargs.get("hermes_home") or "")
        self._platform = str(kwargs.get("platform") or "")
        self._agent_context = str(kwargs.get("agent_context") or "primary")
        from .prefetch_timeout import apply_external_prefetch_timeout

        apply_external_prefetch_timeout()
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        Ledger ``duration_ms`` with ``api_calls=0`` so a slow embed is not blamed on digest LLM.
        """
        ...
        t_digest = time.perf_counter() - t0_digest

        # Stage 2: weekly.on_pre_llm_call
        t0_weekly = time.perf_counter()
        ...
        t_weekly = time.perf_counter() - t0_weekly
        ...
        if q and not _MEM_ID_RE.search(q):
            try:
                ...
                guided = None
        t_recall = time.perf_counter() - t0_recall
        ...
        try:
            from .worker_llm import record_worker_usage

            record_worker_usage(
                {
                    "ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                    "plugin": "memory-prefetch",
                    "purpose": "prefetch",
                    "model": "",
                    "config_model": "",
                    "input_tokens": 0,
            ...
            pass
        # Log per-stage timing for diagnostics
        try:
            from .worker_llm import _worker_logger

            _worker_logger.info(
                "[prefetch] digest=%.3fs weekly=%.3fs recall=%.3fs total=%.3fs",
                t_digest,
                t_weekly,
                t_recall,
    ...
    def on_pre_compress(self, messages: List[Dict[str, Any]]) -> str:
        """Hand Bands A–D to Hermes compact as memory_context, not as a user turn.

        The prefetch fence lives on api_content, so the stored transcript Hermes
        passes here cannot be scraped for it. Returning the band string lets the
        compressor place it in memory-provider-context.
        """
        del messages
        return digest.build_recall_injection_context().strip()
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        """Expose digest, weekly, and monthly slash tokens, including suggest, as chat tools.

        Suggest has to be on this schema or the model cannot pass YYYY-MM tokens for a month list.
        """
        ...
        return [
            ...
            },
            {
                ...
                "description": (
                    "Monthly guidance: update, show, or suggest. "
                    "suggest takes one to four YYYY-MM tokens, then the user talks "
                    "to create a skill, add, replace, or remove. "
                    "Pass the same tokens as /monthly (update, show, suggest, help)."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
