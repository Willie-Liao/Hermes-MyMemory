...
async function startServer() {
  ...
  // 9g. Weekly span candidates (digest bridge — explicit/high after validate)
  app.get('/api/weekly/weeks/:week/spans', async (req, res) => {
    try {
      const { week } = req.params;
      if (!isValidWeekKey(week)) {
        return res.status(400).json({ error: 'A valid week code (YYYY-Www) is required.' });
      ...
      if (mode === 'list') {
        const bridged = await callDigestBridge('list_weekly_span_candidates', {
          week_key: week,
        });
        if (!bridged.ok) {
          return res.status(502).json({ error: bridged.error });
        ...
        if (outcome === 'invalid_week') {
          return res.status(400).json({ error: 'Invalid week key.', ...result });
        }
        return res.json(result);
      }

      const bridged = await callDigestBridge('validate_weekly_spans', {
        week_key: week,
      });
      if (!bridged.ok) {
        return res.status(502).json({ error: bridged.error });
      ...
      // Defense in depth: never return low/medium even if bridge regresses.
      const raw = Array.isArray(result.results) ? result.results : [];
      const results = raw.filter((row) => {
        if (!row || typeof row !== 'object') return false;
        const confidence = String((row as { confidence?: string }).confidence || '')
          .toLowerCase();
  ...
  // 9h. Resolve one weekly overdue span (confirm / put_off / set_due_date)
  app.post('/api/weekly/weeks/:week/spans/resolve', async (req, res) => {
    try {
      const { week } = req.params;
      if (!isValidWeekKey(week)) {
        return res.status(400).json({ error: 'A valid week code (YYYY-Www) is required.' });
      ...
      if (!['confirm', 'put_off', 'set_due_date'].includes(action)) {
      ...
      if (body.proposed_valid_to != null) {
        args.proposed_valid_to = String(body.proposed_valid_to);
      }
      if (body.interval != null) {
        args.interval = String(body.interval);
      }
      if (body.due_date != null) {
        args.due_date = String(body.due_date);
      }
      const bridged = await callDigestBridge('resolve_weekly_span', args);
      if (!bridged.ok) {
        return res.status(502).json({ error: bridged.error });
      }
      const result = bridged.result && typeof bridged.result === 'object'
        ? bridged.result as Record<string, unknown>
        : {};
      if (String(result.outcome || '') === 'error') {
